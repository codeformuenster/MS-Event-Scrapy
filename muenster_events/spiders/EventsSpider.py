...
class EventsSpider(scrapy.Spider):
    ...
    start_url = 'https://www.muenster.de/veranstaltungskalender/' \
                'scripts/frontend/suche.php'

    def start_requests(self):
        today = date.today()
        today_german = today.strftime('%d.%m.%Y')
        today_in_a_year = today + timedelta(365)
        today_in_a_year_german = today_in_a_year.strftime('%d.%m.%Y')
        req_start_date = today_german
        req_end_date = today_in_a_year_german

        self.req_start_date = getattr(self, 'start', req_start_date)
        self.req_end_date = getattr(self, 'end', req_end_date)

        print(self.req_start_date)
        print(self.req_end_date)

        if(self.req_start_date is None):
            self.req_start_date = 'today'
        else:
            if (self.req_start_date is not 'today'
                    and self.req_end_date is None):
    ...
    def after_post(self, response):
        ...
        detail_links = response.xpath(
            "//a[text() = 'Details']/@href").extract()
        for href in detail_links:
            categories = response.xpath(
                "//a[@href = '" + href + "']/ancestor::div[@class = 'eintrag ']/preceding-sibling::div[@class = 'kategorie']/text()").extract()
    ...
    def produce_dates(self, raw_datetime):
        ...
        if(len(datetime_parts) > 2):  # if there is a time given
            time = datetime_parts[2].replace(
                'Uhr', '')  # drop unnessary string
        ...
        if(start_time is ''):
            ...
            start_date = datetime.strptime(
                date + ' ' + start_time, '%d.%m.%Y %H.%M').isoformat()

        if(end_time is not ''):
            end_date = datetime.strptime(
                date + ' ' + end_time, '%d.%m.%Y %H.%M').isoformat()

        return (start_date, end_date)

    def extract_event(self, response):
        """ Callback function for the detail pages. We find the indivudal data
            points and try to bring the date/time in proper form, then
        ...
        link = response.xpath(
            "//div[@class='detail-link']/a/@href").extract_first().strip(' \t\n\r')
        pos = response.xpath(
            "//input[@name='pos']/@value").extract_first().strip(' \t\n\r')
